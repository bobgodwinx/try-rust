...
pub fn run() {
    ...
    let bob_data_2 = BobData { some_bool: false, some_float: 20.0, ..bob_data_1};
    let employee_01 = Employee { first_name: String::from("Bob"), last_name: String::from("ob
    i"), age: 23};
    let employee_02 = Employee { first_name: String::from("Sergi"), last_name: String::from("drapiko"), age: 23};
    println!("{:?}", &bob_data_2);
    println!("{:?}", &employee_01);
    let company = Company { name: String::from("Vodafone"), employees: vec![employee_01, employee_02]};
    println!("{:?}", company);
